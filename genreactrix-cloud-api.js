...
(()=>{'use strict';
 ...
 window.GenreactrixCloudApi=Object.freeze({
  configure(value){base=normalize(value);window.genreactrixSettingsEngine?.set?.('ai.worker.base',base);window.GENREACTRIX_AI_WORKER_BASE=base;return base;},
  getBaseUrl:()=>base,isConfigured:()=>Boolean(base),getKey:()=>window.genreactrixSettingsEngine?.get?.('ai.worker.accessKey','')||localStorage.getItem(KEY_KEY)||'',setKey:value=>window.genreactrixSettingsEngine?.set?.('ai.worker.accessKey',String(value||'')),
