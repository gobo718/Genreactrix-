  ...
  function createEngine(){
    ...
    function commit(imageId,options={}){
      ...
      try{
        ...
        if(rec) window.genreactrixImageRecordEngine.update(imageId,{analysis:{director:{...clone(next),recordedAt:next.updatedAt}},components:{directorReactions:next.reactions.length?"current":"missing",directorThemes:next.themes.some(Boolean)?"current":"missing",primFusion:next.reactions.length>=2?(next.primFusion?"current":"missing"):"current"},attributes:{saved:next.saved,flagged:next.flagged,needsReview:next.flagged},workflow:{stage:next.completion==="complete"?"director-complete":next.completion}},"director-classification-committed");
