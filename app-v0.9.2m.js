const GENREACTRIX_BUILD="v0.9.2m";
const MATRIX_LABEL_FIT = Object.freeze({ preferredPx: 9, stepPx: 0.25, allowedShrinkRatio: 0.15 });
let lastThemeDiagnostic="Diagnostic ready — no Theme save attempted yet.";
function setThemeDiagnostic(message){
  lastThemeDiagnostic=message;
  const themeStatus=$("themeDiagnostic");
  const directorStatus=$("directorStatus");
  if(themeStatus) themeStatus.textContent=message;
  if(directorStatus) directorStatus.textContent=message;
}
function themeSaveDiagnostic(slot,key,saveOk){
  let raw=null;
  try{ raw=localStorage.getItem("genreactrix-v0.9.2j-records"); }catch(_error){}
  let storedRecord=null;
  try{ storedRecord=raw ? JSON.parse(raw)?.[key] : null; }catch(_error){}
  const storedThemes=(storedRecord?.themes||[]).map(themeLabel);
  const activeThemes=(state.themes||[]).map(themeLabel);
  return `Theme ${slot} | key=${key} | save=${saveOk?"OK":"FAILED"} | active=[${activeThemes.join(" / ")}] | stored=[${storedThemes.join(" / ")}] | ${GENREACTRIX_BUILD}`;
}
...
function persistRecords(){
  try{
    ...
    setThemeDiagnostic("Save failed. Classification data was not stored.");
    return false;
  }
}
...
function commitAndAdvance(sourceKey){
  ...
  setThemeDiagnostic(themeSaveDiagnostic(1,sourceKey,saveOk));
  if(!saveOk) return;
  advanceImageIndex();
  ...
  renderAll();
  setThemeDiagnostic(`${lastThemeDiagnostic} | loaded=${destinationKey}`);
}
function navigateImage(delta){
  ...
  loadCurrent();
  const loaded=readClassificationForKey(currentKey());
  setThemeDiagnostic(`Loaded ${currentKey()} | themes=[${loaded.themes.map(themeLabel).join(" / ")}] | v0.9.2m`);
}
...
function renderAll(){
  ...
  if(themeDiagnostic) themeDiagnostic.textContent=lastThemeDiagnostic;
...
function openThemeWorkspace(slot=1){
  ...
  const stored=readClassificationForKey(currentKey());
  setThemeDiagnostic(`Open ${currentKey()} | active=[${state.themes.map(themeLabel).join(" / ")}] | stored=[${stored.themes.map(themeLabel).join(" / ")}] | v0.9.2m`);
...
function selectTheme(themeInput){
  ...
  const saveOk=writeClassificationForKey(sourceKey,classificationState());
  setThemeDiagnostic(themeSaveDiagnostic(sourceSlot,sourceKey,saveOk));
  if(!saveOk) return;
  renderThemes();
  renderComparison();
}
...
window.addEventListener("error",event=>{
  setThemeDiagnostic(`Runtime error: ${event.message} | v0.9.2m`);
});
window.addEventListener("unhandledrejection",event=>{
  const message=event.reason?.message || String(event.reason || "Unknown promise error");
  setThemeDiagnostic(`Runtime error: ${message} | v0.9.2m`);
});
...
try{
  // v0.9.2m uses the verified v0.9.2j storage namespace and with a clean classification namespace and uniquely named assets.
...
// v0.9.2m: hydrate the active demo/image record from persistent storage only
// after all renderer dependencies (including image transform state) exist.
loadCurrent();
setThemeDiagnostic(`Startup ${currentKey()} | active=[${state.themes.map(themeLabel).join(" / ")}] | stored=[${readClassificationForKey(currentKey()).themes.map(themeLabel).join(" / ")}] | v0.9.2m`);
