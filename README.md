## v0.9.39.58 — Image View final centering + compact controls
- Keeps the accepted v0.9.39.57 reaction geometry, scale, ring/emoji proportions, ordering, and vertical placement unchanged.
- Anchors the finished reaction-field bounding box at the literal 50% horizontal/vertical center of the Image View reaction region using absolute centering, eliminating the residual rightward bias from grid self-alignment.
- Compresses the four Image View control buttons to the same accepted 25px height used by the AI drawer controls while preserving the existing 29px control-row bottom edge.
- No changes to Customs CRUD, Judgment geometry, reaction ordering, or Image View theme data.
