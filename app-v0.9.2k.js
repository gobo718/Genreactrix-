const GENREACTRIX_BUILD="v0.9.2j";
...
function themeSaveDiagnostic(slot,key,saveOk){
  let raw=null;
  try{ raw=localStorage.getItem("genreactrix-v0.9.2i-records"); }catch(_error){}
...
function navigateImage(delta){
  ...
  setThemeDiagnostic(`Loaded ${currentKey()} | themes=[${loaded.themes.map(themeLabel).join(" / ")}] | v0.9.2k`);
}
...
function openThemeWorkspace(slot=1){
  ...
  setThemeDiagnostic(`Open ${currentKey()} | active=[${state.themes.map(themeLabel).join(" / ")}] | stored=[${stored.themes.map(themeLabel).join(" / ")}] | v0.9.2k`);
...
window.addEventListener("error",event=>{
  setThemeDiagnostic(`Runtime error: ${event.message} | v0.9.2k`);
});
window.addEventListener("unhandledrejection",event=>{
  const message=event.reason?.message || String(event.reason || "Unknown promise error");
  setThemeDiagnostic(`Runtime error: ${message} | v0.9.2k`);
});
...
try{
  // v0.9.2k uses the verified v0.9.2j storage namespace and with a clean classification namespace and uniquely named assets.
