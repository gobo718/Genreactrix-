...
(()=>{'use strict';
 ...
 async function continueThemeSweepWithValid(id){
  ...
  const sweepEngine=window.genreactrixThemeSweepEngine,sweep=sweepEngine?.get?.(cfg.sweepId),pass=Math.max(1,Math.min(3,Number(cfg.pass)||1)),passState=sweep?.passes?.[String(pass)]||sweep?.passes?.[pass];
