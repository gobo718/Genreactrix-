...
(()=>{'use strict';
 ...
 function eligibleRecords(config){let rows=window.genreactrixImageRecordEngine?.all?.()||[];if(config.target==='flagged')rows=rows.filter(r=>r.attributes.flagged);else if(config.target==='saved')rows=rows.filter(r=>r.attributes.saved);else if(config.target==='failed')rows=rows.filter(r=>r.attributes.failed||Object.values(r.components||{}).includes('failed'));else if(config.target==='current')rows=rows.filter(r=>{const ext=r.metadata?.extended||{},inInbox=Array.isArray(ext.inboxPackIds)&&ext.inboxPackIds.length>0,finished=['batched','red-excluded','hot-magenta-excluded'].includes(r.workflow?.stage);return !inInbox&&!finished&&!r.attributes?.inRecycleBin&&!r.attributes?.archived&&!r.attributes?.rejected;});else if(config.target==='selected'){const ids=new Set((config.imageIds||[]).map(String));rows=rows.filter(r=>ids.has(String(r.id)));}if(config.order==='oldest')rows.sort((a,b)=>a.createdAt.localeCompare(b.createdAt));if(config.order==='newest')rows.sort((a,b)=>b.createdAt.localeCompare(a.createdAt));if(config.order==='random')rows.sort(()=>Math.random()-.5);if(config.quantityMode==='random')rows.sort(()=>Math.random()-.5);return rows;}
