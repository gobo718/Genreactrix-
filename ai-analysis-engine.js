...
(()=>{'use strict';
 ...
 async function processItem(job,item){
  item.state='processing';item.attempts++;item.error='';await put(ITEMS,item);await q()?.setItemState?.(`queue_${item.id}`,'processing',{attempts:item.attempts});window.genreactrixLifecycleEngine?.markAiProcessing?.(item.imageId,{jobId:job.id,attemptId:item.id});job.processing=1;await updateJob(job,{message:`Analyzing ${job.completed+job.failed+1} of ${job.total}`});
  ...
  item.state=errors.length?'failed':'complete';item.error=errors.join(' | ');await put(ITEMS,item);await q()?.setItemState?.(`queue_${item.id}`,item.state,{error:item.error});window.genreactrixLifecycleEngine?.reconcileAfterAi?.(item.imageId,{jobId:job.id,attemptId:item.id});await window.genreactrixBundleEngine?.maybeAutoBundle?.();return item.state;
 }
 ...
 async function run(jobId){
  ...
  for(const item of items){
   ...
   try{const state=await processItem(job,item);job.processing=0;if(state==='complete')job.completed++;else job.failed++}
    ...
    window.genreactrixLifecycleEngine?.reconcileAfterAi?.(item.imageId,{jobId:job.id,attemptId:item.id});
