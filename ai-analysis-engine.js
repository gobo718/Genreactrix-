...
(()=>{'use strict';
 ...
 function serverSingleRequestPlan(job,item,record){
  if(job.config?.themeRerun||job.config?.descriptionRerun)return null;
  ...
  const componentBehaviors=Object.fromEntries(group.map(c=>[c.component,c.behavior])),previous=record.analysis?.ai||{},guidance=String(job.config.analysisGuidance||'').trim().slice(0,6000),existingDescription=String(previous.components?.description||previous.description||'').trim(),existingDescriptionDiagnostics=previous.components?.descriptionDiagnostics&&typeof previous.components.descriptionDiagnostics==='object'?previous.components.descriptionDiagnostics:null,usePreservedMistralDescription=requested.includes('themes')&&!requested.includes('description')&&Boolean(existingDescription)&&existingDescriptionDiagnostics?.thirdProviderUsed===true,providerRouting=window.GenreactrixCloudApi?.getProviderRouting?.()||null;
  const request={imageId:record.id,components:requested,componentBehaviors,promptRefs:job.config.promptRefs||{},directorGuidance:guidance,themeUseAnalysis:Boolean(job.config.themeUseAnalysis),themeAnalysisContext:job.config.themeUseAnalysis?existingDescription.slice(0,6000):'',preservedDescriptionContext:usePreservedMistralDescription?existingDescription.slice(0,12000):'',preservedDescriptionDiagnostics:usePreservedMistralDescription?clone(existingDescriptionDiagnostics):null,descriptionRerun:null,themeRerun:null,themeSweep:clone(job.config.themeSweep||null),providerRouting};
 ...
 async function harvestServerSuccess(job,item,envelope){
  ...
  const previous=record.analysis?.ai||{},guidance=String(job.config.analysisGuidance||'').trim().slice(0,6000),baseMode=artifactEngine.attemptMode({requested,componentBehaviors,themeUseAnalysis:Boolean(job.config.themeUseAnalysis),directReactionUseAnalysis:false,reactionRerunSources:null,directorGuidance:guidance}),mode=`server:${baseMode}`;
  const attempt=await artifactEngine.beginAttempt({imageId:record.id,jobId:job.id,itemId:item.id,itemAttemptId:item.currentAttemptId,components:requested,componentBehaviors,mode,directorGuidance:guidance,inputRefs:{imageId:record.id,sourceKind:'server-job-handoff',serverJobId:job.serverJobId,serverItemId:item.id},configRefs:{projectId:window.genreactrixSettingsEngine?.get?.('project.id')||'',promptRefs:clone(job.config.promptRefs||{}),configuredPromptVersion:window.genreactrixSettingsEngine?.get?.('ai.prompt.version')||'',reactionArchitecture:'theme-derived-six-equal-slots-v1',reactionAiScan:false,serverExecutionProtocol:'d1-r2-queue-v1',serverStartedAt:envelope.startedAt||null,serverCompletedAt:envelope.completedAt||null}});
 ...
 async function harvestServerFailure(job,item,envelope){
  const message=String(envelope?.error||'Server AI analysis failed'),requested=Array.isArray(envelope?.requested)?envelope.requested:(item.components||[]).map(c=>c.component),artifactEngine=window.genreactrixAiArtifactEngine,record=window.genreactrixImageRecordEngine?.get?.(item.imageId,{touch:false});
  let attemptId=null;try{if(artifactEngine&&record){const attempt=await artifactEngine.beginAttempt({imageId:item.imageId,jobId:job.id,itemId:item.id,itemAttemptId:item.currentAttemptId,components:requested,componentBehaviors:Object.fromEntries((item.components||[]).map(c=>[c.component,c.behavior])),mode:'server:failed',directorGuidance:String(job.config.analysisGuidance||'').trim().slice(0,6000),inputRefs:{imageId:item.imageId,serverJobId:job.serverJobId,serverItemId:item.id},configRefs:{serverExecutionProtocol:'d1-r2-queue-v1',providerDiagnostic:clone(envelope?.providerDiagnostic||null)}});attemptId=attempt.id;await artifactEngine.failAttempt(attempt.id,message)}}catch(error){console.warn('Server AI failure attempt could not be recorded',error)}
  for(const c of item.components||[]){c.state='failed';const field=COMPONENTS.find(([id])=>id===c.component)?.[2];if(field&&record)window.genreactrixImageRecordEngine.setComponent(item.imageId,field,'failed')}item.state='failed';item.error=message;await put(ITEMS,item);await q()?.setItemState?.(`queue_${item.id}`,'failed',{error:message});await window.genreactrixHistoryEngine?.append?.({imageId:item.imageId,eventType:'ai-failed',actor:'system',sourceEngine:'ai-analysis',jobId:job.id,summary:message,payload:{attemptId,error:message,components:requested,serverExecution:{protocol:'d1-r2-queue-v1',serverJobId:job.serverJobId,serverItemId:item.id},providerDiagnostic:clone(envelope?.providerDiagnostic||null)}}).catch(()=>{});if(isThemeRerunConfig(job.config))restoreThemeRerunLifecycle(item,'theme-rerun-lifecycle-guard-server-error');else window.genreactrixLifecycleEngine?.reconcileAfterAi?.(item.imageId,{jobId:job.id,attemptId:item.currentAttemptId,error:message,globalFailure:isGlobalProviderFailure(message)});
 }
 ...
 async function runServerJob(job){
  ...
  try{
   if(!job.serverJobId){await updateJob(job,{state:'queued',execution:'server-preparing',message:`Preparing server handoff · ${prepared.manifest.length} images`});const created=await window.GenreactrixCloudApi.createServerJob({clientJobId:job.id,config:{themeSweep:clone(job.config.themeSweep||null),components:clone(job.config.components||{}),siteBuild:window.GENREACTRIX_BUILD||null},items:prepared.manifest}),remote=created?.result||created,serverJobId=remote?.job?.id;if(!serverJobId)throw new Error('Worker did not return a server job id');job.serverJobId=serverJobId;job.execution='server-preparing';await put(JOBS,job);}
