...
(()=>{'use strict';
 ...
 async function run(jobId){
  ...
  if(!stopped&&themeSweepManaged){
   ...
   if(pass<3&&outcome.holdIds.length){
    const nextPass=pass+1,next=sweepEngine.prepareNext(sweep.id,nextPass,outcome.holdIds),components=componentMap();components.themes={enabled:true,behavior:'reanalyze'};if(job.config?.components?.genreReasons?.enabled)components.genreReasons={enabled:true,behavior:'reanalyze'};
    const nextConfig={target:'selected',imageIds:[...next.imageIds],quantityMode:'all',quantity:next.imageIds.length,order:'queue',components,promptRefs:clone(job.config.promptRefs||{}),themeSweep:{managed:true,sweepId:sweep.id,pass:nextPass,orderMode:'shuffled',orderSeed:next.orderSeed,rootJobId:sweep.rootJobId||job.id}};
 ...
 async function forceStagedQueueToThemeSweepPass2(){
  ...
  if(!ids.length){const el=document.getElementById('aiJobSummary');if(el)el.textContent='No Staged Queue images are available for Pass 2.';return;}
  const ok=window.confirm(`Send all ${ids.length} Staged Queue image${ids.length===1?'':'s'} directly to Theme Sweep Pass 2? Themes will rerun using one new shuffled Theme order fixed for this entire pass.`);if(!ok)return;
  const engine=window.genreactrixThemeSweepEngine;if(!engine?.beginPass2FromStaged)throw new Error('Theme Sweep Pass 2 recovery control is unavailable');
  const prior=engine.latest?.(),bootstrap=engine.beginPass2FromStaged({imageIds:ids,sourceSweepId:prior?.id||null});
  if(!bootstrap?.id)throw new Error('Could not create Theme Sweep Pass 2 recovery');
  const components=componentMap();components.themes={enabled:true,behavior:'reanalyze'};
  const cfg={target:'selected',imageIds:[...ids],quantityMode:'all',quantity:ids.length,order:'queue',components,promptRefs:{},themeSweep:{managed:true,sweepId:bootstrap.id,pass:2,orderMode:'shuffled',orderSeed:bootstrap.orderSeed,rootJobId:''}};
  const job=await createJob(cfg);if(!job?.id)throw new Error('Could not queue the Staged Queue population for Pass 2');
  engine.attachPassJob?.(bootstrap.id,2,job.id,ids,bootstrap.orderSeed);window.genreactrixThemeSweepEngine?.render?.();
  const el=document.getElementById('aiJobSummary');if(el){el.dataset.jobId=job.id;el.textContent=`Theme Sweep Pass 2 queued for ${ids.length} Staged Queue images.`;}
  await run(job.id);
 }
 ...
 async function render(){
  ...
  const forcePass2=document.getElementById('aiThemeSweepForceStagedPass2');if(forcePass2){const staged=Number(window.genreactrixLifecycleEngine?.snapshot?.().staged)||0;forcePass2.disabled=staged<1;forcePass2.textContent=staged?`Send ${staged} Staged to Pass 2`:'Send Staged to Pass 2';forcePass2.title=staged?`Rerun Themes for all ${staged} currently Staged Queue images as Theme Sweep Pass 2 using one fixed shuffled Theme order.`:'No Staged Queue images are available.';}
 ...
 function initUi(){
  ...
  document.getElementById('aiThemeSweepForceStagedPass2')?.addEventListener('click',()=>forceStagedQueueToThemeSweepPass2().catch(e=>{const el=document.getElementById('aiJobSummary');if(el)el.textContent=String(e?.message||e);console.error(e);}));
