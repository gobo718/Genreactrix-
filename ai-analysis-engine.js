...
(()=>{'use strict';
 ...
 async function run(jobId){
  ...
  if(!stopped&&themeSweepManaged){
   ...
   const outcome=sweepEngine.evaluate(passIds,pass);sweepEngine.finishPass(sweep.id,pass,outcome);
   ...
   finalMessage+=pass<3?(outcome.holdIds.length?` · Theme Sweep pass ${pass}: ${outcome.holdIds.length} held${tripletLabel?` for ${tripletLabel}`:''}`:` · Theme Sweep pass ${pass}: all released`):` · Theme Sweep verification: ${outcome.releaseIds.length} released${outcome.failedIds.length?` · ${outcome.failedIds.length} unresolved`:''}`;
