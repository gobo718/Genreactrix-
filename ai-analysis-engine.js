...
(()=>{'use strict';
 ...
 async function finalizeServerLocalJob(job){
  ...
  if(!stopped&&themeSweepManaged){const sweepEngine=window.genreactrixThemeSweepEngine,pass=Math.max(1,Math.min(3,Number(sweepConfig.pass)||1)),sweep=sweepEngine?.get?.(sweepConfig.sweepId),passIds=sweep?.passes?.[String(pass)]?.imageIds||sweep?.passes?.[pass]?.imageIds||[];if(!sweepEngine||!sweep)throw new Error('Theme Sweep state is unavailable');const outcome=sweepEngine.evaluate(passIds,pass);if(outcome.failedIds.length){sweepEngine.blockPassForFailures?.(sweep.id,pass,{...outcome,imageIds:[...passIds]});finalMessage+=` · Theme Sweep Pass ${pass} blocked: ${outcome.successful}/${outcome.analyzed} valid · ${outcome.failedIds.length} failed · use Retry Failed`;await updateJob(job,{state:'completed-with-failures',completedAt:now(),message:finalMessage});await q()?.setJobState?.(`queue_${job.id}`,'completed-with-failures',finalMessage);render();return;}sweepEngine.finishPass(sweep.id,pass,outcome);try{await window.genreactrixBundleEngine?.maybeAutoBundle?.();}catch(error){console.error('Theme Sweep release Bundle check failed',error)}const tripletLabel=outcome.triplet?.labels?.join(' / ')||outcome.triplet?.codes?.join(' / ')||'';finalMessage+=pass<3?(outcome.holdIds.length?` · Theme Sweep pass ${pass}: ${outcome.holdIds.length} held${tripletLabel?` for ${tripletLabel}`:''}`:` · Theme Sweep pass ${pass}: all released`):` · Theme Sweep verification: ${outcome.releaseIds.length} released`;await updateJob(job,{state:finalState,completedAt:now(),message:finalMessage});await q()?.setJobState?.(`queue_${job.id}`,finalState,finalMessage);render();if(pass<3&&outcome.holdIds.length){const nextPass=pass+1,next=sweepEngine.prepareNext(sweep.id,nextPass,outcome.holdIds),components=componentMap();components.themes={enabled:true,behavior:'reanalyze'};if(job.config?.components?.genreReasons?.enabled)components.genreReasons={enabled:true,behavior:'reanalyze'};if(job.config?.themeSweep?.persistDescription)components.description={enabled:true,behavior:'reanalyze'};const nextConfig={target:'selected',imageIds:[...next.imageIds],quantityMode:'all',quantity:next.imageIds.length,order:'queue',components,promptRefs:clone(job.config.promptRefs||{}),themeSweep:{managed:true,sweepId:sweep.id,pass:nextPass,orderMode:'shuffled',orderSeed:next.orderSeed,rootJobId:sweep.rootJobId||job.id,persistDescription:Boolean(job.config?.themeSweep?.persistDescription)}};const nextJob=await createJob(nextConfig);if(nextJob?.id){sweepEngine.attachPassJob(sweep.id,nextPass,nextJob.id,next.imageIds,next.orderSeed);await run(nextJob.id);}else console.warn('Theme Sweep recovery pass could not be queued',nextJob);}else setTimeout(()=>maintainActiveMode().catch(console.warn),0);return;}
  if(!stopped&&!isThemeRerunConfig(job.config)){try{const bundles=await maybeBundleAfterAi(),staged=window.genreactrixLifecycleEngine?.snapshot?.().staged||0;if(bundles.length)finalMessage+=` · ${bundles.reduce((n,b)=>n+(b.imageIds?.length||0),0)} Bundled to Inbox`;else if(staged)finalMessage+=` · ${staged} Staged in Queue`;}catch(error){finalMessage+=` · Bundle check failed: ${String(error?.message||error)}`;console.error('Automatic Bundle check failed',error)}}await updateJob(job,{state:finalState,completedAt:now(),message:finalMessage});await q()?.setJobState?.(`queue_${job.id}`,finalState,finalMessage);render();if(!stopped&&!isThemeRerunConfig(job.config))setTimeout(()=>maintainActiveMode().catch(console.warn),0);
 }
