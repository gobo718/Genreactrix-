...
(()=>{'use strict';
 ...
 const api={all,byId,activeBundles,stagedRecords,bundleSize,autoFlowEnabled,create,bundleStaged,bundleWhateverAvailable,maybeAutoBundle,contains,bundleIdsForImage,finalizeBatchImages,migrateRecordMemberships,snapshot};
 window.genreactrixBundleEngine=api;
 window.addEventListener('DOMContentLoaded',()=>{try{migrateRecordMemberships();emit('ready',null,{snapshot:snapshot()});}catch(error){console.error('Bundle engine initialization failed',error)}});
})();
