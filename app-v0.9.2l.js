const GENREACTRIX_BUILD="v0.9.2l";
...
function navigateImage(delta){
  ...
  setThemeDiagnostic(`Loaded ${currentKey()} | themes=[${loaded.themes.map(themeLabel).join(" / ")}] | v0.9.2l`);
}
...
function openThemeWorkspace(slot=1){
  ...
  setThemeDiagnostic(`Open ${currentKey()} | active=[${state.themes.map(themeLabel).join(" / ")}] | stored=[${stored.themes.map(themeLabel).join(" / ")}] | v0.9.2l`);
...
window.addEventListener("error",event=>{
  setThemeDiagnostic(`Runtime error: ${event.message} | v0.9.2l`);
});
window.addEventListener("unhandledrejection",event=>{
  const message=event.reason?.message || String(event.reason || "Unknown promise error");
  setThemeDiagnostic(`Runtime error: ${message} | v0.9.2l`);
});
...
try{
  // v0.9.2l uses the verified v0.9.2j storage namespace and with a clean classification namespace and uniquely named assets.
