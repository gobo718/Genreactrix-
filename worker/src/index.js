/* Genreactrix AI Worker v0.9.6.43-semantic-calibration
   Registry-driven replacement Worker.
   Source vocabulary is generated from primfusion-registry.json.
*/
const API_VERSION = '0.9.6.43-semantic-calibration';
...
function parsePromptDiagnosticPartial(raw,expected){
  ...
  const setPart=(code,part,assessment,reason)=>{
    code=String(code||'').toUpperCase();
    part=Number(part);assessment=promptDiagnosticCanonicalAssessment(assessment);reason=String(reason||'').trim().replace(/^[-–—:|\s]+/,'');
