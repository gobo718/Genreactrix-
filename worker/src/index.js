/* Genreactrix AI Worker v0.9.6.158-theme-taxonomy-update
...
*/
const API_VERSION = '0.9.6.158-theme-taxonomy-update';
...
- Do not convert generic positive/evaluative language into Theme evidence. “Visually appealing,” “interesting,” “striking,” “compelling,” “evocative,” “thought-provoking,” “well-balanced,” “elegant,” or “beautifully composed” do not by themselves imply Goofy, Cozy, Sassy, Dreamy, Nostalgia, Poignant, Romance, or any other affective Theme.
...
function freshDescriptionPrompt({directorGuidance='',preliminaryThemes=[],workspaceContext=''}={}){
  const guidance=String(directorGuidance||'').trim().slice(0,6000),workspace=String(workspaceContext||'').trim();
...
async function runFreshThemeAwareDescription(env,model,image,{behavior='analyze',directorGuidance='',preliminaryThemes=[],workspaceContext=''}={}){
  const prompt=freshDescriptionPrompt({directorGuidance,preliminaryThemes,workspaceContext});
  const zazzly=zazzlyDescriptionProtocol(preliminaryThemes);
  const cycle=await runThemeProviderCycle(env,model,{
...
function descriptionRerunRegistryThemes(descriptionRerun=null){
  const rerun=normalizeDescriptionRerun(descriptionRerun);if(!rerun)return[];
  const byName=new Map(PRIMFUSION_REGISTRY.aiThemeChoices.map(row=>[String(row.name||'').trim().toLowerCase(),row]));
  const seen=new Set(),rows=[];
  for(const item of rerun.themes){
    const row=byName.get(String(item.label||'').trim().toLowerCase());
    if(!row||seen.has(row.code))continue;
    seen.add(row.code);rows.push({rank:rows.length+1,code:row.code,name:row.name});
  }
  return rows;
}

function descriptionRerunWorkspaceContext(descriptionRerun=null){
  const rerun=normalizeDescriptionRerun(descriptionRerun);if(!rerun)return'';
  ...
  return `LANDSCAPE DESCRIPTION RERUN CONTEXT.\nSelected Themes and historical Descriptions are attention/reference context, not facts that must be preserved. Keep observations only when the image supports them.\n\nSELECTED THEME CONTEXT:\n${themeBlock}\n\nSELECTED DESCRIPTION CONTEXT:\n${descriptionsBlock}`;
}
...
function descriptionPrompt(directorGuidance="",descriptionRerun=null){
  ...
  if(!rerun)return freshDescriptionPrompt({directorGuidance:guidance,preliminaryThemes:[]});
  if(rerun.operation==='all')throw new Error('Rewrite All must use the unified v5 Description pipeline.');
  const target=rerun.targetDescription;
  if(rerun.operation==='add'&&(!target||!target.text.trim()))throw new Error('Description Add rerun requires a populated target Description');
  if(rerun.operation==='replace'&&(!target||!target.text.trim()||target.end<=target.start))throw new Error('Description Replace rerun requires a highlighted target span');
  const selectedThemes=descriptionRerunRegistryThemes(rerun),zazzly=zazzlyDescriptionProtocol(selectedThemes),workspace=descriptionRerunWorkspaceContext(rerun);
  const operationBlock=rerun.operation==='add'
    ? `OPERATION: ADD AT CURSOR.\nThe target Description is shown below with ⟦CURSOR⟧ at the exact insertion point. Return ONLY the new prose fragment that should be inserted there. Everything else is immutable.\n\nTARGET DESCRIPTION:\n${target.text.slice(0,target.start)}⟦CURSOR⟧${target.text.slice(target.start)}`
    : `OPERATION: REPLACE HIGHLIGHTED SECTION.\nThe target Description is shown below with the exact editable span marked. Return ONLY replacement prose for the highlighted span. Everything outside the markers is immutable.\n\nTARGET DESCRIPTION:\n${target.text.slice(0,target.start)}⟦HIGHLIGHT START⟧${target.text.slice(target.start,target.end)}⟦HIGHLIGHT END⟧${target.text.slice(target.end)}`;
  return `GENREACTRIX DESCRIPTION SCOPED EDIT — V5 CONTENT STANDARD.\n\nThis is a surgical edit to an existing Genreactrix Description, not a generic rewrite. Match the existing Description's specificity, directness, terminology, concreteness, and level of detail. Do not flatten it into bland summary prose. Do not sanitize, euphemize, genericize, or tone down explicit but visibly supported content.\n\nUse the same content-coverage standard as the full Genreactrix Description pass where it is relevant to the requested edit: be comfortable describing people, bodies, clothing or nudity, poses, expressions, actions, physical contact, setting, objects, visible text, composition, and overall presentation. Sexual, erotic, violent, humiliating, grotesque, disturbing, or otherwise explicit content may be important evidence and should be stated directly and neutrally when supported.\n\nINDEPENDENT SEXUAL-SALIENCE CHECK — apply this to the editable subject matter even when no selected Theme is Zazzly. Examine body presentation, exposed skin or nudity, underwear or revealing clothing, pose, gaze or expression, framing, and public or deliberate body display. If the requested insertion/replacement concerns imagery reasonably read as sexy, erotic, provocative, flirtatious, exhibitionistic, suggestive, or body-focused, say so directly. Nudity alone does not prove sexual intent.${zazzly.text}\n\nSCOPED EDIT BOUNDARY — the coverage rules above do NOT authorize rewriting or summarizing the whole Description. Generate only the text required at the marked edit location, while matching the surrounding prose.\n\n${operationBlock}\n\n${workspace}${guidance?`\n\nDIRECTOR GUIDANCE:\n${guidance}`:''}\n\nReturn only the ${rerun.operation==='add'?'insertion':'replacement'} fragment as plain prose. Do not repeat unchanged surrounding text. Do not explain the edit. Do not wrap the answer in JSON, Markdown code fences, or a field label.`;
}
...
async function analyze(env,body){
  ...
  const themeTask=themeFamilyRequested?(async()=>{
    ...
    if(themeRerun){
      if(themeHumanVoteRerunExperimentEligible(themeRerun)){
        ...
        promptVersions.themes='genreactrix-themes-pfm-v28-rerun-human-vote-raw-full66-badass-tuning';
      }else{
      ...
      promptVersions.themes='genreactrix-themes-pfm-v39-mistral-primary-whole-run-full66-badass-tuning';
    }
  ...
  if (requested.includes('description')){
    ...
    if(!descriptionRerun&&components.__freshPipelineDescription){
      ...
      promptVersions.description='genreactrix-freeform-v5-independent-sexual-salience-zazzly-exhaustive';
      ...
      promptVersions.description='genreactrix-freeform-v5-independent-sexual-salience-zazzly-exhaustive';
    }else if(descriptionRerun.operation==='all'){
      const preliminaryRun=await runPreliminaryThemeMismatchRecovery(env,model,image,behavior,body.themeSweep||null),preliminary=preliminaryRun.result;
      const preliminaryThemes=resolveThemes(preliminary.selections),selectedContextThemes=descriptionRerunRegistryThemes(descriptionRerun),seen=new Set();
      const attentionThemes=[];
      for(const row of [...preliminaryThemes,...selectedContextThemes]){if(row?.code&&!seen.has(row.code)){seen.add(row.code);attentionThemes.push(row);}}
      const workspaceContext=descriptionRerunWorkspaceContext(descriptionRerun),descriptionPass=await runFreshThemeAwareDescription(env,model,image,{behavior,directorGuidance:body.directorGuidance,preliminaryThemes:attentionThemes,workspaceContext});
      components.description=descriptionPass.description;
      components.descriptionDiagnostics={...descriptionPass.diagnostics,rerunWorkspace:true,rerunOperation:'all',generatedPreliminaryThemes:preliminaryThemes.map(row=>({rank:row.rank,code:row.code,name:row.name})),selectedContextThemes:selectedContextThemes.map(row=>({code:row.code,name:row.name})),preliminaryMismatchRecovery:preliminaryRun.recovery};
      promptVersions.description='genreactrix-freeform-v5-rerun-workspace-all-unified';
    }else{
      const description = await runStructured(env,model,image,descriptionPrompt(body.directorGuidance,descriptionRerun),descriptionSchema(),3400,'text',{behavior,scopedEdit,preserveWhitespace:scopedEdit});
      ...
      components.descriptionDiagnostics={schemaVersion:1,protocol:'description-rerun-v5-scoped-edit-style-match',rerunWorkspace:true,rerunOperation:descriptionRerun.operation,selectedContextThemes:descriptionRerunRegistryThemes(descriptionRerun).map(row=>({code:row.code,name:row.name}))};
      promptVersions.description = `genreactrix-freeform-v5-rerun-workspace-${descriptionRerun.operation}-style-matched`;
    }
...
async function controlServerAiJob(env,jobId,action,options={}){
  if(!serverJobBindingsReady(env))throw new Error('Server AI jobs are not fully configured');
  ...
  async fetch(request,env={}){
    ...
    try{
      ...
      if(serverJobPath){
        ...
        if(url.pathname.endsWith('/control')&&request.method==='POST'){const body=await request.json().catch(()=>({}));return json({ok:true,result:await controlServerAiJob(env,jobId,String(body?.action||''),body||{})});}
