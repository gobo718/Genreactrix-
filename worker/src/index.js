/* Genreactrix AI Worker v0.9.6.111-goofy-camp-swap
   Preserves Mistral description rescue and downstream provider recovery.
   Preserves bounded source-page image resolution and production cleanup. Matrix remains 0.0.0.0.
   Preserves the approved six-Theme definition calibration from v0.9.6.104.
   Keeps Theme diagnostics fatal, but retries one failed fresh image Theme/Description/Theme-Info chain from scratch on the alternate provider after a malformed diagnostic exhausts its immediate format retry.
*/
const API_VERSION = '0.9.6.111-goofy-camp-swap';
...
async function runThemeReasoningDiagnostic(env,model,image,behavior,selections=[],themeSweep=null){
  ...
  for(let attempt=1;attempt<=2;attempt++){
    try{
      ...
      if(validPrimCount<PRIMFUSION_REGISTRY.primitives.length||validAuditCount<selections.length)throw new Error(`Theme reasoning diagnostic incomplete: ${validPrimCount}/${PRIMFUSION_REGISTRY.primitives.length} Prim scores and ${validAuditCount}/${selections.length} selected audits.`);
    ...
    try{
      ...
      return{
        ...
        diagnostics:{
          ...
          frozenControlWorker:'0.9.6.84-theme-exhaustion-slop-warning',
          themeDefinitionOrder:'fixed-shuffled-78-v1',
          imageAccess:true,
...
async function analyze(env,body){
  ...
  if (requested.includes('themes') || requested.includes('genreReasons')){
    ...
    if(themeRerun){
      ...
      // Preserve the proven fresh-image Theme machinery. If a Theme reasoning
      // diagnostic remains malformed after its existing immediate format retry,
      // reject that image's current fresh Theme/Description/Theme-Info attempt
      // and rerun the whole fresh chain once from scratch on the alternate provider.
      const runFreshThemeChain=async(runEnv)=>{
        ...
        if(descriptionPass.diagnostics?.thirdProviderUsed){
          try{
            const downstream=await runThemesFromMistralDescriptionThroughExistingProviders(runEnv,model,image,behavior,{description:descriptionPass.description,themeSweep:body.themeSweep||null});
          ...
          initialFinalDecision=await runThemeAssociation(runEnv,model,{description:descriptionPass.description,behavior,themeSweep:body.themeSweep||null,stage:'final'});
          mismatchRecovery=await runFreshThemeMismatchRecovery(runEnv,model,image,behavior,{description:descriptionPass.description,initialDecision:initialFinalDecision,themeSweep:body.themeSweep||null});
        }
        return{
      ...
      const mainTrace=providerTrace(env),traceCheckpoint=Array.isArray(mainTrace)?mainTrace.length:null;
      let freshChain;
      try{
        freshChain=await runFreshThemeChain(env);
      }catch(error){
        const firstDiagnostic=providerDiagnosticOf(error)||{};
        const malformedDiagnostic=firstDiagnostic.phase==='theme-reasoning-diagnostic-incomplete'&&firstDiagnostic.failureKind==='malformed-or-incomplete';
        if(!malformedDiagnostic)throw error;
        if(Array.isArray(mainTrace)&&traceCheckpoint!==null)mainTrace.splice(traceCheckpoint);
        const alternate=alternateProviderEnv(env);
        try{
          freshChain=await runFreshThemeChain(alternate.env);
          mergeProviderTrace(env,alternate.env,'theme-diagnostic-whole-run-alternate-retry');
          freshChain.themeDecisionDiagnostics.wholeRunAlternateProviderRetry={schemaVersion:1,trigger:'theme-reasoning-diagnostic-incomplete',discardedInitialAttempt:true,from:alternate.from,to:alternate.to,succeeded:true};
        }catch(retryError){
          mergeProviderTrace(env,alternate.env,'theme-diagnostic-whole-run-alternate-retry-failed');
          const retryDiagnostic=providerDiagnosticOf(retryError)||{};
          throw diagnosticError(retryError?.message||'Alternate-provider Theme/Description/Theme-Info retry failed.',{
            ...retryDiagnostic,
            wholeRunAlternateProviderRetryExhausted:true,
            discardedInitialAttempt:true,
            initialFailurePhase:firstDiagnostic.phase||'theme-reasoning-diagnostic-incomplete',
            initialFailureKind:firstDiagnostic.failureKind||'malformed-or-incomplete',
            alternateProviderFrom:alternate.from,
            alternateProviderTo:alternate.to
          });
        }
      }
      ...
      promptVersions.themes='genreactrix-themes-pfm-v31-diagnostic-alternate-whole-run-retry';
    }
