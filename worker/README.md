# Genreactrix v0.9.40.103 — Prompt Diagnostics Component-Heading Recovery

## v0.9.40.103

- Bundles Worker **0.9.6.51-prompt-diagnostics-component-heading-recovery**.
- Prompt Diagnostics component repairs now recognize the provider form `CODE.PART :: component text` followed by separate `ASSESSMENT: <token>` and `REASON: <text>` lines.
- Recovery is deliberately narrow: the heading must contain an exact expected component ID, the following substantive line must be a structured `ASSESSMENT` using the allowed vocabulary, and the following reason is attached only to that same pending component. Pending heading state is discarded on any unrelated substantive line or new component so it cannot bleed across concepts or components.
- The `REASON:` label is normalized as formatting rather than stored as part of the semantic reason.
- Regression coverage includes the observed P01.07–P01.11 response shape, existing one-line component output, existing assessment-on-component/reason-next-line output, unrelated-prose rejection, and cross-component bleed rejection.
- No Prim/PrimFusion definition, scoring, Theme-selection, reaction, layout, or geometry changes. Matrix remains **0.0.0.0**.
- Inherits v0.9.40.102 unlabeled-WHY recovery, v0.9.40.101 Playful/Snarky calibration, and v0.9.40.100 browser transport recovery unchanged.
