...
function activeInboxRecords(){return (recordEngine()?.all?.()||[]).filter(record=>inboxRecord(record)&&!record?.attributes?.inRecycleBin&&!record?.attributes?.archived&&!record?.attributes?.parked&&!alreadyBatched(record))}
function pendingInboxRecords(){return activeInboxRecords().filter(record=>classified(record)&&eligible(record))}
...
function inboxWorkSnapshot(){
  const records=activeInboxRecords(),pending=pendingInboxRecords(),pendingIds=new Set(pending.map(r=>r.id));
  return{
    ...outcomeCountsForRecords(pending),
    total:records.length,
    ready:pending.length,
    remaining:records.filter(r=>!pendingIds.has(r.id)).length,
    saved:records.filter(r=>r.attributes?.saved).length,
    flagged:records.filter(r=>r.attributes?.flagged).length,
    blocked:records.filter(r=>!eligible(r)).length,
    imageIds:records.map(r=>r.id)
  };
}
...
async function existingSubmissionReport(batchId,submissionVersion){return (await reportsForBatch(batchId)).find(r=>Number(r.submissionVersion||0)===Number(submissionVersion))||null}
async function submit(id,{allowFlagged=true}={}){
  ...
  const records=b.imageIds.map(imageId=>recordEngine()?.get(imageId,{touch:false})).filter(Boolean);
  const locked=records.filter(r=>r.attributes?.locked);
  if(locked.length)throw new Error(`${locked.length} image(s) are locked; Batch made no changes`);
  await update(id,{status:'submitting',submissionAttempt:{version:nextVersion,startedAt:submittedAt,imageIds:[...b.imageIds],outcomes:preOutcomes}},'batch-submit-started');
  let report=await existingSubmissionReport(id,nextVersion);
  if(!report)report=window.genreactrixReportsEngine?.generateStandardBatchReport?await window.genreactrixReportsEngine.generateStandardBatchReport(b.id,nextVersion):standardReport(b);
  if(!window.genreactrixReportsEngine&&!await get(REPORTS,report.id))await put(REPORTS,report);
  for(const imageId of b.imageIds){
    ...
    if(r.attributes?.rejected)recordEngine().update(imageId,{...shared,workflow:{stage:'rejected'}},'batch-reject-submitted');
    else recordEngine().update(imageId,{...shared,workflow:{stage:'batched'}},'batch-submitted');
  }
  const uncommitted=b.imageIds.filter(imageId=>{
    const r=recordEngine()?.get(imageId,{touch:false});
    return !r||r.metadata?.extended?.lastBatchSubmissionId!==b.id||!r.timestamps?.batchedAt||(r.attributes?.rejected?r.workflow?.stage!=='rejected':r.workflow?.stage!=='batched');
  });
  if(uncommitted.length)throw new Error(`Batch commit incomplete for ${uncommitted.length} image(s); submission remains resumable`);
  const inboxCleanup=await window.genreactrixInboxLifecycle?.finalizeBatchImages?.(b.imageIds,{batchId:b.id,submittedAt})||{removedImages:0,removedPacks:0};
  const submitted=await update(id,{status:'submitted',submittedAt,completedAt:submittedAt,submissionVersion:nextVersion,submissionAttempt:null,reportIds:[...new Set([...(b.reportIds||[]),report.id])]},'batch-submitted');
  let recycled=0;
  for(const imageId of b.imageIds){
    const r=recordEngine()?.get(imageId,{touch:false});
  ...
  await history()?.append?.({imageId:null,eventType:'batch-report-generated',actor:'system',sourceEngine:'batch',batchId:id,summary:'Standard batch report generated',payload:{reportId:report.id,counts:report.counts,recycled,outcomes:preOutcomes,inboxCleanup}}).catch(()=>{});
  await create({name:'New Batch',imageIds:[],activate:true});
  window.genreactrixNotificationsEngine?.create?.({severity:'info',title:'Batch submitted',message:`${b.name} · ${b.imageIds.length} images · ${preOutcomes.reject} rejected · ${recycled} recycled`,ownerEngine:'batch',relatedBatchId:id,relatedReportId:report.id,actionTarget:'batch',actionLabel:'Batch',dedupeKey:`batch:${id}:submission:${nextVersion}`,resolved:true});
  emit();return{submitted,report,recycled,outcomes:preOutcomes,inboxCleanup};
}
...
async function snapshot(){const b=await active();snapshotCache={activeBatch:b,activeBatchId:b?.id||null,counts:b?counts(b):{total:0,ready:0,rejected:0,incomplete:0,partial:0,flagged:0,saved:0,inaccessible:0,submitted:0,remaining:0},outcomes:b?outcomeCounts(b):outcomeCountsForRecords([]),pending:pendingInboxSnapshot(),inboxWork:inboxWorkSnapshot(),batches:await all(),reports:b?await reportsForBatch(b.id):[]};engine.activeIdCached=snapshotCache.activeBatchId;engine.snapshotCached=snapshotCache;return structuredClone(snapshotCache)}
...
async function resumeInterruptedSubmission(){
  const interrupted=(await all()).find(batch=>batch.status==='submitting');
  if(!interrupted)return null;
  await setActive(interrupted.id);
  try{return await submit(interrupted.id)}catch(error){console.warn('Interrupted Batch submission still needs attention',error);return null}
}
