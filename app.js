const GENREACTRIX_BUILD="v0.9.40.126";
...
// v0.9.40.126 — adaptive AI AMA interview chunks (9 -> 3 -> 1) with persistent checkpointing; Tuned/SLOP retained.
...
function amaMissingQuestionIds(run,blockIndex){const saved=new Set((run?.questions||[]).map(q=>String(q?.id||'').toUpperCase()).filter(Boolean));return amaBlockQuestionIds(run,blockIndex).filter(id=>!saved.has(id))}
function amaAdaptiveProviderError(error){const status=Number(error?.httpStatus)||0,text=String(error?.message||error||'').toLowerCase();return status===429||status>=500||/timed?\s*out|timeout|rate limit|temporar|unavailable|workers ai vision failed|provider call/i.test(text)}
function amaStageLabel(stage,blockIndex,run,meta={}){if(stage==='visual')return'Reading image';if(stage==='candidates')return'Auditing candidate Themes';if(stage==='questions'){const blockCount=Math.max(1,Number(run?.blockCount)||Math.ceil((Number(run?.questionCount)||68)/(Number(run?.blockSize)||9))),ids=Array.isArray(meta?.questionIds)&&meta.questionIds.length?meta.questionIds:null,range=ids?{label:amaQuestionIdsLabel(ids)}:amaBlockRange(run,blockIndex),recovery=meta?.recoveryLevel&&meta.recoveryLevel!=='primary'?` · ${meta.recoveryLevel}`:'';return`Interview block ${Number(blockIndex)+1}/${blockCount} · ${range.label}${recovery}`}if(stage==='finalize')return'Finalizing report';return'AI AMA'}
...
async function runAmaCurrent(){
  ...
  let run=await engine.latestIncompleteRun(record.id),currentStage='created',currentBlock=null,currentQuestionIds=[],currentRecoveryLevel='primary';
  ...
  try{
    if(!String(run.visualRead||'').trim()){
      currentStage='visual';currentQuestionIds=[];currentRecoveryLevel='primary';setAmaRunProgress('Preparing image for AI visual read… · 0/68 saved');setDirectorStatus('AI AMA · preparing image…');if(runBtn)runBtn.textContent='Reading image…';
    ...
    if(!run.candidatesReady){
      currentStage='candidates';currentQuestionIds=[];currentRecoveryLevel='primary';if(runBtn)runBtn.textContent='Auditing Themes…';amaStartStatusClock(`Auditing candidate Themes · ${amaRunProgressText(run)}`);setDirectorStatus('AI AMA · visual read saved · auditing candidate Themes…');
    ...
    for(let blockIndex=0;blockIndex<blockCount;blockIndex++){
      ...
      const callChunk=async(ids,recoveryLevel)=>{
        const requested=[...ids];currentQuestionIds=requested;currentRecoveryLevel=recoveryLevel;const progress=engine.runProgress(run),label=amaQuestionIdsLabel(requested),levelText=recoveryLevel==='primary'?'primary block attempt':recoveryLevel==='3-question recovery'?'3-question recovery':'single-question recovery';
        if(runBtn)runBtn.textContent=`Running ${label}…`;amaStartStatusClock(`Block ${blockIndex+1}/${blockCount} · ${label} · ${progress.answered}/${progress.expected} saved · ${levelText}`,{sameSizeRetryPossible:false});setDirectorStatus(`AI AMA · block ${blockIndex+1}/${blockCount} · ${label} · ${levelText} · ${progress.answered}/${progress.expected} saved…`);
        try{
          ...
          run=await engine.saveQuestionBlock(run.id,{questions:result.questions||[],blockIndex,complete:false,meta:{workerVersion:result.workerVersion||run.workerVersion,matrixVersion:result.matrixVersion||run.matrixVersion,model:result.model||run.model,questionCount:Number(result.questionCount)||run.questionCount||68,blockSize:Number(result.blockSize)||run.blockSize||9,blockCount:Number(result.blockCount)||run.blockCount||blockCount,questionBlocks:result.blocks||run.questionBlocks||[],lastError:null}});
          const after=engine.runProgress(run),stillMissing=requested.filter(id=>amaMissingQuestionIds(run,blockIndex).includes(id));if(stillMissing.length)setAmaRunProgress(`${label} returned partial results · ${after.answered}/${after.expected} saved · missing ${stillMissing.join(', ')} · reducing only the missing work.`);else setAmaRunProgress(`${label} saved · ${after.answered}/${after.expected} core Q&A saved.`);
          return{result,missing:stillMissing};
        }catch(error){amaStopStatusClock();throw error}
      };
      const savedBefore=blockIds.length-missing.length,priorFailedThisBlock=run.lastError?.stage==='questions'&&Number(run.lastError?.blockIndex)===blockIndex;
      if(savedBefore===0&&missing.length>3&&!priorFailedThisBlock){
        try{await callChunk(missing,'primary')}catch(error){if(!amaAdaptiveProviderError(error))throw error;const progress=engine.runProgress(run);setAmaRunProgress(`${amaQuestionIdsLabel(missing)} failed at primary size · ${progress.answered}/${progress.expected} saved · switching immediately to 3-question recovery. ${amaCompactError(error)}`);setDirectorStatus(`AI AMA · ${range.label} primary attempt failed · switching to 3-question recovery.`)}
        missing=amaMissingQuestionIds(run,blockIndex);
      }else if(savedBefore===0&&missing.length>3&&priorFailedThisBlock){const progress=engine.runProgress(run);setAmaRunProgress(`Prior failure already recorded for ${range.label} · ${progress.answered}/${progress.expected} saved · skipping another full-block wait and starting 3-question recovery.`);setDirectorStatus(`AI AMA · ${range.label} previously failed · starting 3-question recovery.`)}
      const groups=[];for(let i=0;i<blockIds.length;i+=3)groups.push(blockIds.slice(i,i+3));
      for(const group of groups){
        let groupMissing=group.filter(id=>amaMissingQuestionIds(run,blockIndex).includes(id));if(!groupMissing.length)continue;
        if(groupMissing.length>1){
          try{await callChunk(groupMissing,'3-question recovery')}catch(error){if(!amaAdaptiveProviderError(error))throw error;const progress=engine.runProgress(run);setAmaRunProgress(`${amaQuestionIdsLabel(groupMissing)} failed at recovery size · ${progress.answered}/${progress.expected} saved · switching only this group to single-question recovery. ${amaCompactError(error)}`);setDirectorStatus(`AI AMA · ${amaQuestionIdsLabel(groupMissing)} failed · switching to single questions.`)}
          groupMissing=group.filter(id=>amaMissingQuestionIds(run,blockIndex).includes(id));
        }
        for(const id of groupMissing){await callChunk([id],'single-question recovery')}
      }
      missing=amaMissingQuestionIds(run,blockIndex);if(missing.length){const progress=engine.runProgress(run);throw new Error(`AI AMA block ${blockIndex+1}/${blockCount} (${range.label}) is still incomplete. Missing ${missing.join(', ')}. ${progress.answered}/${progress.expected} answers were saved and can be resumed.`)}
      run=await engine.saveQuestionBlock(run.id,{questions:[],blockIndex,complete:true,meta:{lastError:null}});const progress=engine.runProgress(run);setAmaRunProgress(`Block ${blockIndex+1}/${blockCount} complete · ${progress.answered}/${progress.expected} core Q&A saved.`);
    }
...
// v0.9.40.126 — adaptive resumable AI AMA / Tuned / SLOP Director diagnostics.
