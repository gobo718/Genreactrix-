const GENREACTRIX_BUILD="v0.9.39.21";
...
// v0.9.39.21 — structured customs, editing/reordering, and evaluation-vocabulary versioning.
...
function cleanupPlaceholderCustoms(){if(localStorage.getItem(CUSTOM_PLACEHOLDER_CLEANUP_KEY)==="1")return;const placeholders=new Set(["horror","dreamcore","horny","none"]);saveCustomThemes((state.customThemes||[]).filter(x=>!placeholders.has(String(x.label||"").trim().toLowerCase())));saveCustomReactions((state.customReactions||[]).filter(x=>!placeholders.has(String(x.label||"").trim().toLowerCase())));localStorage.setItem(CUSTOM_PLACEHOLDER_CLEANUP_KEY,"1");}
...
function wireLibraryReorder(node,kind,id){let timer=0,active=false;const stop=()=>{clearTimeout(timer);if(active){active=false;node.classList.remove("reordering");document.body.classList.remove("custom-reorder-active");}};node.addEventListener("pointerdown",e=>{timer=setTimeout(()=>{active=true;node.classList.add("reordering");document.body.classList.add("custom-reorder-active");node.setPointerCapture?.(e.pointerId);navigator.vibrate?.(25);},CUSTOM_REORDER_HOLD_MS);});node.addEventListener("pointermove",e=>{if(!active)return;e.preventDefault();const target=document.elementFromPoint(e.clientX,e.clientY)?.closest?.(`.landscape-custom-item[data-kind="${kind}"]`);if(target&&target!==node)reorderCustomLibrary(kind,id,target.dataset.id);});node.addEventListener("pointerup",stop);node.addEventListener("pointercancel",stop);node.addEventListener("lostpointercapture",stop);}
