...
async function amaNormalizeThreeQuestionRun(engine,run){if(!run||amaThreeQuestionPlanMatches(run))return run;const plan=amaThreeQuestionPlan(run?.questionCount||68),saved=new Set((run?.questions||[]).map(q=>String(q?.id||'').toUpperCase()).filter(Boolean)),completed=plan.blocks.filter(block=>block.questionIds.every(id=>saved.has(id))).map(block=>block.index);return engine.checkpointRun(run.id,{questionCount:plan.questionCount,blockSize:3,blockCount:plan.blockCount,questionBlocks:plan.blocks,completedBlockIndexes:completed,executionStrategy:'3-to-1',lastError:null})}
    ...
    for(let blockIndex=0;blockIndex<blockCount;blockIndex++){
      ...
      const callChunk=async(ids,recoveryLevel)=>{
        ...
        try{
          ...
          run=await engine.saveQuestionBlock(run.id,{questions:result.questions||[],blockIndex,complete:false,meta:{workerVersion:result.workerVersion||run.workerVersion,matrixVersion:result.matrixVersion||run.matrixVersion,model:result.model||run.model,questionCount:Number(result.questionCount)||run.questionCount||68,blockSize:3,blockCount:Number(result.blockCount)||run.blockCount||blockCount,questionBlocks:result.blocks||run.questionBlocks||[],executionStrategy:'3-to-1',lastError:null}});
          const after=engine.runProgress(run),missingSet=new Set(amaMissingQuestionIds(run,blockIndex)),stillMissing=requested.filter(id=>missingSet.has(id));if(stillMissing.length)setAmaRunProgress(`${label} returned partial results · ${after.answered}/${after.expected} saved · missing ${stillMissing.join(', ')} · switching only the missing question(s) to single-question recovery.`);else setAmaRunProgress(`${label} saved · ${after.answered}/${after.expected} core Q&A saved.`);
