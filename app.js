const GENREACTRIX_BUILD="v0.9.39.20";
...
function moveDraftReaction(chip,target){const from=Number(chip.dataset.index),to=Number(target.dataset.index);if(from===to||!Number.isInteger(from)||!Number.isInteger(to)||from<0||to<0||from>=customThemeDraft.reactionRefs.length||to>=customThemeDraft.reactionRefs.length)return;const[item]=customThemeDraft.reactionRefs.splice(from,1);customThemeDraft.reactionRefs.splice(to,0,item);const parent=chip.parentElement;if(from<to)parent.insertBefore(chip,target.nextSibling);else parent.insertBefore(chip,target);[...parent.querySelectorAll(".custom-expression-chip")].forEach((node,i)=>node.dataset.index=String(i));updateCustomDialogValidation();}
...
function openCustomReactionDialog(){if($("customReactionLabel"))$("customReactionLabel").value="";if($("customReactionEmoji"))$("customReactionEmoji").value="";if($("customReactionStatus"))$("customReactionStatus").textContent="";updateCustomDialogValidation();$("customReactionDialog")?.showModal();$("customReactionLabel")?.focus();}
function openCustomThemeDialog(){customThemeDraft.reactionRefs=[];if($("customThemeLabel"))$("customThemeLabel").value="";if($("customThemeStatus"))$("customThemeStatus").textContent="";renderCustomThemePicker();$("customThemeDialog")?.showModal();$("customThemeLabel")?.focus();}
function saveCustomReactionFromDialog(){const validation=customReactionValidation();if(!validation.valid){updateCustomDialogValidation(true);return;}const label=$("customReactionLabel").value.trim(),emoji=$("customReactionEmoji").value.trim(),id=`custom-reaction:${slugifyCustom(label)}`;saveCustomReactions([...(state.customReactions||[]),normalizeCustomReaction({id,label,emoji})]);$("customReactionDialog")?.close();renderAll();renderCustomThemePicker();}
function saveCustomThemeFromDialog(){const validation=customThemeValidation();if(!validation.valid){updateCustomDialogValidation(true);return;}const label=$("customThemeLabel").value.trim(),id=`custom-theme:${slugifyCustom(label)}`;saveCustomThemes([...(state.customThemes||[]),normalizeCustomThemeRecord({id,label,reactionRefs:customThemeDraft.reactionRefs})]);$("customThemeDialog")?.close();renderWriteIns();renderAll();}
function renderLandscapeCustoms(){const themeList=$("tabletCustomThemeList"),reactionList=$("tabletCustomReactionList"),q=$("tabletCustomSearch")?.value.trim().toLowerCase()||"";if(themeList){themeList.innerHTML="";(state.customThemes||[]).filter(item=>!q||item.label.toLowerCase().includes(q)).forEach(theme=>{const b=document.createElement("button");b.type="button";b.textContent=theme.label;b.addEventListener("click",()=>{if(tabletLandscapeView.activeThemeSlot===null)return;state.targetSlot=tabletLandscapeView.activeThemeSlot;selectTheme(theme);});themeList.appendChild(b);});}if(reactionList){reactionList.innerHTML="";(state.customReactions||[]).filter(item=>!q||item.label.toLowerCase().includes(q)||item.emoji.includes(q)).forEach(record=>{const token=customReactionSelectionToken(record.id),b=document.createElement("button");b.type="button";b.className=state.selectedReactions.includes(token)?"selected":"";b.innerHTML=`<span>${record.emoji}</span> ${record.label}`;b.addEventListener("click",()=>{pushHistory();const n=state.selectedReactions.indexOf(token);if(n>=0)state.selectedReactions.splice(n,1);else state.selectedReactions.push(token);saveCurrent("director-custom-reaction-auto");renderAll();});reactionList.appendChild(b);});}}
...
document.querySelectorAll("[data-close-custom-dialog]").forEach(button=>button.addEventListener("click",()=>$(button.dataset.closeCustomDialog)?.close()));
