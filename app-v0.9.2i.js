const GENREACTRIX_BUILD="v0.9.2i";
...
function persistRecords(){
  try{
    ...
    localStorage.setItem("genreactrix-v0.9.2i-records",recordsJson);
    localStorage.setItem("genreactrix-v0.9.2i-ai-runs",aiRunsJson);
    // Verify the classification write immediately. A failed or blocked write must
    // not masquerade as a successful save in the UI.
    if(localStorage.getItem("genreactrix-v0.9.2i-records")!==recordsJson){
...
function commitAndAdvance(sourceKey){
  ...
  renderAll();
  const status=$("directorStatus");
  if(status) status.textContent=`Theme 1 saved for ${sourceKey}. Loaded ${destinationKey}. ${GENREACTRIX_BUILD}`;
}
function nextImage(){
  advanceImageIndex();
  loadCurrent();
...
function selectTheme(themeInput){
  ...
  renderComparison();
  const status=$("directorStatus");
  if(status) status.textContent=`Theme ${sourceSlot} saved for ${sourceKey}. ${GENREACTRIX_BUILD}`;
}
...
try{
  // v0.9.2i intentionally starts with a clean classification namespace and uniquely named assets.
  ...
  const currentRecords=localStorage.getItem("genreactrix-v0.9.2i-records");
  state.records=currentRecords?JSON.parse(currentRecords):{};
  state.aiRuns=JSON.parse(localStorage.getItem("genreactrix-v0.9.2i-ai-runs")||"{}");
